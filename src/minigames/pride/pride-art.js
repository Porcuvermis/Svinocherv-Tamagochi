...
const PRIDE_ART = {
    ...
    Z_START: 22,     // глубина, на которой в НАЧАЛЕ выхода стоит арка входа, —
                     // и она же дальний конец расстеленного ковра. Ближе
                     // ставить нельзя: у самой камеры арка перестаёт быть
                     // воротами и превращается в красную полосу поперёк кадра
    CAR_Z: 16,       // глубина, на которой машина стоит в НАЧАЛЕ выхода — чуть
                     // позади червя, он только что из неё вышел. Дальше она
                     // остаётся позади и уезжает к горизонту вместе со всем,
                     // что он прошёл. Ближе ставить нельзя: машина ВДВОЕ шире
                     // дорожки и вплотную к камере закрывает собой всю сцену
    ...
    scene() {
        ...
        <!-- КОВЁР — ЭТО И ЕСТЬ ШКАЛА ВЫХОДА.
             Он расстелен от арки входа до камеры, и его дальний конец
             привязан к арке: червь идёт на зрителя, вход остаётся позади, и
             красная полоса за его спиной становится всё длиннее. К концу
             выхода она дотягивается до горизонта.
    ...
    // ---------- АРКА ВХОДА ----------
    // Ворота, из которых червь вышел на дорожку. Она и есть шкала выхода, но
    // читается СЗАДИ: в начале арка стоит прямо над червём, дальше остаётся
    // позади и уменьшается, а когда добралась до горизонта — дорожка пройдена.
    // Отдельного счётчика 0..100 в игре нет (docs/plan/17-pride.md, раздел 4).
    //
    // Почему шкала позади, а не впереди: червь идёт НА зрителя, и всё, к чему
    // он идёт, находится за камерой — впереди показывать нечего. Зато
    // пройденное видно целиком, и «докуда дошёл» читается по тому, как далеко
    // уехал вход.
    arch() {
        const C = PALETTE.redCarpet;
        const w = 820, h = 1150;
        ...
        <g>
            ...
            <path d="M ${-w} ${-h} L ${w} ${-h} L ${w} ${-h + 200} L ${-w} ${-h + 200} Z"
                  fill="${C.kiss[700]}" stroke="${C.rail[300]}" stroke-width="14"/>
            <path d="M ${-w + 60} ${-h + 240} Q 0 ${-h + 400} ${w - 60} ${-h + 240}"
                  fill="none" stroke="${C.kiss[300]}" stroke-width="30"/>
        </g>`;
    },
