...
//   мыло    — мылиться быстрее и дольше оставаться чистым: шкала похоти
//             пустеет медленнее (12 → 24 ч);
//   мочалка — тереть быстрее и раньше снова играть на жетон: таймер
//             награды короче (6 → 3 ч).
//
// ---------- ПОКУПАЕТ НЕ ЭКРАН ----------
...
// толчки за финал, у мыла и мочалки — клетки под одним касанием и, второй
// строкой со значком часов, часы.
...
const LustShop = {
    ...
    value(key, level) {
        ...
        return Math.round(Math.PI * v.radius * v.radius);
    },

    // Вторая половина мыла и мочалки — ЧАСЫ, и она важнее первой: мыло
    // растягивает, сколько червь остаётся чистым (шкала пустеет за drain
    // часов), мочалка сокращает, через сколько снова доступна награда
    // (cooldown). Строка вторая и мельче: часы — это то, что игрок
    // почувствует между заходами, а клетки — то, что увидит в самом забеге.
    // Значок часов, а не подпись (инвариант 9); чьи это часы, говорит
    // картинка полки.
    hoursHtml(key, level) {
        if (key !== 'soap' && key !== 'cloth') return '';
        const b = this.conf()[key];
        const at = (l) => (l <= 0 ? b.base : (b.levels[Math.min(l, b.levels.length) - 1] || {}).bonus) || {};
        const f = key === 'soap' ? 'drain' : 'cooldown';
        const fmt = (h) => (Math.round(h * 10) / 10).toString();
        return `<span class="ls-step ls-hours"><em>⏱</em>
                    <b>${fmt(at(level)[f])}</b><i>→</i><b class="ls-next">${fmt(at(level + 1)[f])}</b>
                </span>`;
    },
