...
const WrathLobby = {
    ...
    init(host) {
        ...
        // Шрамы могут смениться, пока лобби открыто: их перерабатывает
        // удержание на черве, и их же добавляет debug-панель. Шина вместо
        // прямого вызова — потому что живых экземпляров персонажа может быть
        // два сразу, главный экран и открытая мини-игра (src/core/worm.js).
        if (typeof GameEvents !== 'undefined') {
            GameEvents.on('worm-marks', (e) => {
                if (!this.wormHandle) return;
                this.wormHandle.setOverride({ scars: (e && e.scars) || [] });
            });
        }
    ...
    HOLD_MS: 700,
    BURST_MS: 420,      // пик вспышки: на нём шрамы и стираются
    ...
    finishHold() {
        ...
        // должны вылететь оттуда, где они были, а не из середины экрана.
        const from = this.scarPoints();

        if (this.wormBox) this.wormBox.classList.add('cleansed');
        if (typeof Haptics !== 'undefined') Haptics.notify('success');

        // Искры летят в счётчик жетонов — туда же, куда через миг прилетит
        // цифра «+1» (src/core/wallet-fx.js). Порядок такой: сначала искры,
        // потом начисление, иначе цифра успевает отыграть раньше, чем до неё
        // долетели.
        if (typeof WalletFx !== 'undefined') WalletFx.sparks('wrath_token', from);

        // Тело перерисовывается НЕ сразу, а на пике вспышки: к этому моменту
        // шрамы уже выгорели добела и стали искрами, и перерисовка их просто
        // не находит. Кадра здесь мало — при перерисовке через кадр шрамы
        // исчезали раньше, чем вспышка успевала начаться, и выглядело это как
        // подмена картинки, а не как её работа.
        // Перерисовываются ВСЕ экземпляры — и главный экран тоже.
    ...
    // координатах. Отсюда вылетают искры, поэтому меряется по РЕАЛЬНЫМ узлам,
    // а не по модели: узел уже стоит там, куда его поставил рендерер со всеми
