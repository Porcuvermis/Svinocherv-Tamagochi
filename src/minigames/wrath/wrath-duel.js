...
const WrathDuel = {
    ...
    enter(mode, order) {
        ...
        // Максимум передаётся потому, что зарастание считается долей от него:
        // без максимума заморозка недосчитала бы накопленное у прокачанного
        // бойца (Backend.freezeHeal).
        if (!this.order) Backend.freezeHeal(WrathFighter.playerHp().max);
