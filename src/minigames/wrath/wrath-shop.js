...
const WrathShop = {
    ...
    // ---------- КОШЕЛЁК ----------
    // Слева — сколько жетонов собрано, справа — сколько собрано СЛЕДУЮЩЕГО.
    //
    // Раньше здесь стояла пара значков со счётчиками: «🎟 2» и «🩸 1/3». Она
    // говорила число, но не говорила главного — сколько осталось. Дробь надо
    // прочитать и додумать, а до этого ещё догадаться, что 🩸 и 🎟 вообще
    // связаны между собой: связь была написана в конфиге и нигде на экране.
    //
    // Теперь оба — нарисованный жетон (src/core/token-art.js). Недостающие
    // дольки видны пунктиром, то есть расстояние до цели показано, а не
    // сосчитано. Дробь при этом не нужна вовсе: картинка И ЕСТЬ дробь.
    //
    ...
    walletHtml(lack) {
        const item = (key, inner) =>
            `<span class="wallet-item${lack === key ? ' lack' : ''}" data-cur="${key}">${inner}</span>`;
        // Целый жетон и число рядом: это кошелёк, а не прогресс.
        const whole = TokenArt.svg('wrath_token', 0, { whole: true });
        // Недособранный. Сколько в нём закрашено, считает TokenArt по осколкам
        // в кошельке и по накопленным поражениям — экран правило не повторяет.
        const partial = TokenArt.svg('wrath_token',
            TokenArt.progress('wrath_token', 'wrath.duel.lose'));
        return item('wrath_token', `${whole}<b>${GameState.currency('wrath_token')}</b>`)
             + item('wrath_shard', partial);
    },
