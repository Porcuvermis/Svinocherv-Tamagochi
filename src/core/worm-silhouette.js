...
function wormSkullHalf(cfg, yaw, side) {
    ...
    // ---------- СКУЛА — САМОЕ ШИРОКОЕ МЕСТО, А НЕ УГОЛ ----------
    // Первая управляющая точка второго сегмента раньше стояла своей
    // шириной (`cheek * 0.99 * lo`). Касательные в стыке от этого не
    // совпадали: сверху кривая подходила к скуле почти отвесно, а снизу
    // сразу отваливала наружу — на контуре получался ИЗЛОМ.
    //
    // При анфасе он был 4° и читался скулой. Но множители `up` и `lo`
    // расходятся с поворотом (верх со стороны лица уже, низ полнее), и
    // излом рос вместе с ракурсом: 16° на половине поворота, 32° на полном.
    // На затылке это выглядело срезанным углом — будто голову отрубили.
    //
    // Лечится не подбором числа, а тем, что точка ПРОДОЛЖАЕТ касательную
    // предыдущего сегмента. Длина взята так, чтобы она осталась на прежней
    // высоте (y = 0.3): форма та же, угла нет. Умножение на радиусы —
    // аффинное преобразование, оно параллельность сохраняет, поэтому
    // сводить касательные можно прямо здесь, в долях.
    const cheekX = k * cheek * up;
    // Касательная первого сегмента в его конце: (p − c2).
    const tanX = cheekX - k * temple * up, tanY = -0.08 - (-0.72);
    const CHEEK_RUN = (0.3 - (-0.08)) / tanY;   // столько касательной, чтобы дойти до y = 0.3
    return [
        // свод → виски → скулы
        { c1: [k * brow * up, -0.99], c2: [k * temple * up, -0.72], p: [cheekX, -0.08] },
        // скулы → брыли → морда
        { c1: [cheekX + tanX * CHEEK_RUN, 0.3], c2: [k * jaw * lo, 0.56], p: [k * muzzle * lo, 0.84] },
        // морда → подбородок
