...
const WormMood = {
    ...
    // Здесь только «насколько сер», а КАК он сереет — дело рендерера.
    //
    // Способ менялся дважды, и оба раза не от вкуса. Сначала стояла запись
    // style.filter — на айфоне она молча не работала: WebKit не применяет CSS
    // filter к внутренним элементам SVG. Потом стоял родной SVG-фильтр — он
    // работал везде, но стоил двух третей кадров: фильтр на слое персонажа
    // заставляет браузер каждый кадр гонять весь слой через отдельный буфер,
    // а червь меняется каждый кадр. Теперь обесцвечивание ЗАПЕЧЕНО в сами
    // цвета узлов (worm-renderer.js, witherColor и bakeWither).
    //
    // Что осталось прежним: сереет только ЧЕРВЬ, а не его жильё — комната
    // обязана оставаться цветной, она точка отсчёта, по которой и видно, что
    // червю плохо. И переход по-прежнему сглаживает рендерер сам.
    setGrey(saturation) {
