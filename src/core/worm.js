...
const WormMood = {
    ...
    // Обесцвечивание — фильтром, а не подменой цветов в модели. Причина:
    ...
    //
    // Сам фильтр живёт в рендерере и висит на слоях ЧЕРВЯ, а не на всей
    // сцене: истощён персонаж, а не его жильё. Комната обязана оставаться
    // цветной — она точка отсчёта, по которой и видно, что червю плохо.
    //
    // Здесь остался только вызов. Раньше в этом месте стояла запись
    // style.filter, и на айфоне она молча не работала: WebKit не применяет
    // CSS filter к внутренним элементам SVG. Теперь рендерер делает это
    // родным SVG-фильтром, а заодно сам сглаживает переход.
    setGrey(saturation) {
        const shown = Math.round(saturation * 100) / 100;
        if (this.shownGrey === shown) return;
        this.shownGrey = shown;
        if (MainWormHandle.setWither) MainWormHandle.setWither(shown);
    }
};
...
function initWorm() {
    try {
        ...
        MainWormHandle = window.WormRenderer.mount(container, model, {
            context: 'main',
            // Персонаж пересчитывается через кадр, а не каждый. Он самая
            // дорогая вещь на экране — семьсот узлов, и по ним каждый раз
            // проходит вся анимация; на медленном телефоне комната из-за
            // него одного шла на двадцати пяти кадрах. Тридцать обновлений
            // в секунду для ходьбы и покачивания хватает с запасом, а сцена
            // получает вдвое больше воздуха (docs/traps.md, п. 36).
            frameHz: 30,
