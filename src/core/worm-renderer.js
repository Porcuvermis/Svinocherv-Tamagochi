...
const WormRenderer = {
    ...
    mount(container, model, opts) {
        ...
        function rebuild() {
            ...
            // ---------- РАЗЛОЖИТЬ ТЕЛО СРАЗУ ----------
            // Из сборщика персонаж выходит СЛОЖЕННЫМ: где стоят звенья
            // цепочки, какой у тела силуэт и какого он цвета — всё это
            // доводит tick(). Значит каждая пересборка показывала кадр с
            // телом, схлопнутым к началу координат. Замер: высота сразу
            // после пересборки 112 точек против 213 через три кадра.
            //
            // Поодиночке (сменил косметику, открыл мини-игру) это вспышка на
            // кадр, и её годами не замечали. Но при правке ползунком
            // пересборок два десятка в секунду — и персонаж буквально
            // стробоскопит. Разбор — docs/traps.md, п. 140.
            //
            // Кадровый цикл при этом НЕ раздваивается: текущий запрос
            // снимается, а вынужденный кадр сам ставит следующий.
            if (state.rafId) {
                cancelAnimationFrame(state.rafId);
                state.rafId = 0;
                state.lastFrameTs = 0;      // dt = 0: время не проматывается
        ...
        // force — разложить тело ПРЯМО СЕЙЧАС, вне очереди кадров. Нужно
        // после пересборки: из сборщика персонаж выходит сложенным, и
        // ждать следующего кадра значит показать его таким.
            ...
            if (state.built) {
                ...
                if ((state.built.wearOnBody && state.built.wearOnBody.length)
                    ...
                    for (let k = 0; k < sw.length; k++) {
                        ...
                        // Носителей три: тело (не поворачивается вовсе), хвост
                        // (его угол считает tick) и ухо (свой наклон, живой —
                        // он меняется с ракурсом головы).
                        const host = s2.ear ? (s2.ear.baseAngle || 0)
                                   : s2.part === 'tail' ? (state.tailWearRot || 0) : 0;
                        // s2.ang — куда уехал свободный конец (плюс вправо), а
                        // поворот носителя вычитается, чтобы висящее держало
                        // свой угол по ЭКРАНУ.
                        //
                        // ---------- ЗЕРКАЛО МЕНЯЕТ ЗНАК ПОВОРОТА ----------
                        // Левая серьга живёт внутри scale(-1,1): там
                        // положительный поворот виден на экране отрицательным.
                        // Пока множителя не было, серьги на одном завале
                        // телефона обе съезжались к голове, а на другом
                        // расходились — вместо того чтобы обе искать низ.
                        // Угол носителя НЕ зеркалится: он уже экранный.
                        const want = (s2.flip || 1) * (-s2.ang - host);
                        if (Math.abs(want - s2.now) < 0.2) continue;
                        s2.now = want;
                        setAttr(s2.el, 'transform', s2.pre + want.toFixed(1) + s2.post);
                    }
                }
        ...
        return {
            ...
            // Модель ПОСЛЕ наложения оверрайда — то есть та, по которой
            // персонаж нарисован прямо сейчас, а не сохранённая базовая.
            // Разница не теоретическая: в лобби гнева оверрайд подменяет
            // половину внешности, и инспектор, спросивший loadWormModel(),
            // показывал бы числа не того червя, на которого смотрят.
            //
            // Снимок, а не ссылка: mergeWormOverride базу не мутирует и
            // возвращает новый объект, поэтому правка возвращённого ничего
            // не меняет на экране. Менять надо через setOverride/setLivePose.
            getModel() {
                return mergedModel();
            },
            // Чужой оверрайд — тот, что поставила мини-игра. Нужен пульту:
            // setOverride ЗАМЕНЯЕТ патч целиком, а не сливает, и пульт,
            // положивший свой патч не глядя, стёр бы подмену внешности,
            // которую делает лобби гнева. Своё он кладёт ПОВЕРХ этого.
            getOverride() {
                return state.override || null;
            },
            // ---------- МОДЕЛЬ БЕЗ ОВЕРРАЙДА ----------
            // Исходная, «как задумано». Нужна пульту, и без неё он считал
            // от ТОГО, ЧТО САМ ЖЕ НАПИСАЛ: ползунок стоял на +0.5, а голова
            // на каждом его движении множилась ещё раз — 1.5, 2.25, 3.375.
            // Относительная ручка обязана отсчитывать от неподвижной точки,
            // иначе она не ручка, а педаль газа.
            getBaseModel() {
                return state.baseModel;
            },
            // Живые каналы целиком — их значения не лежат в модели, а
            // мини-игра могла перехватить любой (см. setLivePose).
            getLivePose() {
                return Object.assign({}, state.livePose);
            },
            setPose(name) {
