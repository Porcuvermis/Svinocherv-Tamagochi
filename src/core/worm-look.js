...
const WormLook = {
    ...
    push(handle, opts) {
        ...
        // Совпало с тем, что УЖЕ на этом персонаже — отдавать нечего. Но и
        ...
        //
        // «На ЭТОМ» — не формальность: червей на экране несколько (комната,
        // студия, ванная), а патч у пульта один. Без сверки по экземпляру
        // правка, сделанная в студии, не доезжала до комнатного червя: числа
        // те же, значит «уже применено» — и студия закрывалась впустую.
        if (handle === this.lastHandle && json === this.lastJson) { this.pending = null; return; }
