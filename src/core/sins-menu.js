...
// течёт энергия и копится в узле. Дошло до порога голода
// (ECONOMY.sins.<грех>.payAt, у всех семи — половина шкалы) — узел
// загорается: мини-игра теперь платит полную награду.
//
...
const SinsMenu = {
    ...
    update(force) {
        ...
        ECONOMY.sinOrder.forEach(key => {
            ...
            // Горит ли узел, решает не меню, а тот же, кто и платит: правило
            // порога живёт в одном месте (инвариант 2), иначе колесо однажды
            // начнёт обещать награду, которой не будет.
            const ready = Backend.sinPays(key);
