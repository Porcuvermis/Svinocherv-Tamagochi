...
// самом червя (0.5 с) и занимает весь экран: в центре логотип игры, из него
...
// течёт энергия и копится в узле. Набралось столько, что за заход заплатят,
// — узел загорается.
//
// ---------- ЭТО КАРКАС, А НЕ ЗАКОН ----------
// Сейчас все семь читаются ОДИНАКОВО — по шкале и порогу голода, потому что
// ничего другого пока и нет. Но одинаково они жить не будут: у лени награда
// идёт за выращенное, а не за «прошёл мини-игру» — конца у неё вообще нет; у
// алчности автомат будет крутиться бесконечно. Что в таких грехах означает
// «налито» и «горит», решается вместе с самой мини-игрой, а не заранее.
//
// Поэтому чтение греха вынесено в подменяемую функцию (`readers` ниже), а не
// зашито в перерисовку. Появился грех со своим законом — рядом ложится его
// читалка, остальные шесть не трогаются. Общего правила «горит по половине
// шкалы» в коде колеса нет и заводить его не надо.
//
...
const SinsMenu = {
    ...
    // Длина удержания. Полсекунды: столько палец готов ждать, не решив, что
    // игра его не слышит. Меньше — меню открывалось бы от обычного тапа по
    // червю (а тап по нему уже значит «повозиться»).
    HOLD_MS: 500,
    ...
    bindTaps() {
        ...
        this.svg.addEventListener('pointerup', (e) => {
            // Палец подняли — отсчёт удержания кончился, чем бы он ни был.
            // Без этой строки одиночный тап по логотипу закрывал меню: кольцо
            // продолжало наливаться уже без пальца и через полсекунды
            // срабатывало само.
            this.cancelHold();
            const g = e.target.closest && e.target.closest('.sm-sin');
    ...
    startHold(e, x, y, done, atCore) {
        ...
        // Сторож на ВСЁ окно, а не только на тот элемент, где палец опустился.
        // Причина конкретная: палец может уехать с элемента, захват указателя
        // может забрать себе рендерер комнаты, а отпускание может прийти
        // вообще мимо меню — и любой такой случай оставлял бы отсчёт жить без
        // пальца. Отсчёт обязан кончаться вместе с касанием, где бы оно ни
        // кончилось. Ловим в фазе перехвата: так нас не обойдёт ничей
        // stopPropagation.
        const stop = (ev) => {
            if (this._hold && ev.pointerId === this._hold.id) this.cancelHold();
        };
    ...
    // Снять отсчёт, не трогая кольца: одно и то же и когда удержание
    // отменили, и когда оно дошло до конца.
    dropHold() {
        const h = this._hold;
        if (!h) return;
        this._hold = null;
        clearTimeout(h.timer);
        window.removeEventListener('pointerup', h.stop, true);
        window.removeEventListener('pointercancel', h.stop, true);
    },
    ...
    // ---------- ЧИТАЛКИ ГРЕХОВ ----------
    // Как превратить грех в две величины, которые умеет показать колесо:
    //
    //     { fill: 0…100, ready: true|false }
    //
    // fill  — насколько налит луч (энергия, скопившаяся в узле);
    // ready — горит ли узел, то есть заплатят ли за заход прямо сейчас.
    //
    // Таблица пустая НАРОЧНО: пока все семь читаются одинаково, и класть сюда
    // семь копий одной функции незачем. Когда у греха появится свой закон —
    // сад платит за выращенное, автомат алчности крутится без конца — его
    // читалка ложится сюда одной записью:
    //
    //     SinsMenu.readers.sloth = () => ({ fill: …, ready: … });
    //
    // и ни колесо, ни остальные шесть грехов об этом не узнают. Читалка живёт
    // рядом со своей мини-игрой, а не здесь: что считать готовностью, знает
    // она, а не меню.
    readers: {},

    read(key) {
        const own = this.readers[key];
        const view = (typeof own === 'function' ? own(key) : this.readDefault(key)) || {};
        const fill = Math.max(0, Math.min(100, Number(view.fill) || 0));
        return { fill: Math.round(fill * 10) / 10, ready: !!view.ready };
    },

    // Умолчание: шкала греха и порог голода. По жиле течёт НЕХВАТКА — полная
    // шкала даёт пустой луч, просевшая налитый: копится ровно то, за чем
    // игрок сюда придёт.
    //
    // Горит ли узел, меню не считает само, а СПРАШИВАЕТ у того, кто платит
    // (инвариант 2). Иначе картинка однажды начнёт обещать награду, которой
    // не будет.
    readDefault(key) {
        ...
        return {
            ...
            ready: (typeof Backend !== 'undefined') ? Backend.sinPays(key) : false
        };
    },
    ...
    update(force) {
        ...
        ECONOMY.sinOrder.forEach(key => {
            ...
            const view = this.read(key);
            const shown = view.fill;
