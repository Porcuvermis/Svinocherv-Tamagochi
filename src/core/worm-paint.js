...
// ---------- ДАЛЬШЕ ГОЛОВА НЕ ПОВОРАЧИВАЕТСЯ ----------
// Предел ОДИН на игру и на редактор. Раньше его не существовало: автоповорот
// доходил до 0.5, именованные позы left/right стояли на 0.5, а ползунок
// ракурса в студии ходил до 1 — то есть редактор показывал ракурсы, которых
// в игре не бывает (42° против 21°). Правили внешность по ним, а выглядит
// она в игре иначе.
//
// Держится здесь, рядом с автоповоротом, чтобы эти два числа нельзя было
// развести: предел и есть тот угол, до которого доворачивается голова сама.
const WORM_HEAD_YAW_LIMIT = WORM_HEAD_YAW_FOLLOW;

function wormClampYaw(v) {
    const L = WORM_HEAD_YAW_LIMIT;
    return Math.max(-L, Math.min(L, v || 0));
}
