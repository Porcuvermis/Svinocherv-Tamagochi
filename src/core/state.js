...
const GameState = {
    ...
    createInitial() {
        ...
        const state = {
            ...
            //
            // frozen — «регенерация сейчас не идёт». Ставится на время боя и
    ...
    // боями оно восстанавливается само, и считается это формулой — как значение шкалы
    ...
    // Хп в минуту: база из конфига плюс прокачка. Плоское число, а не доля
    // от максимума, — тогда ветка «+здоровье» и ветка «регенерация» спорят
    // друг с другом честно (разбор — ECONOMY.minigames.wrath).
    regenPerMinute() {
        const base = (ECONOMY.minigames.wrath && ECONOMY.minigames.wrath.regenPerMinute) || 0;
        return base + this.upgradeBonus('regen');
    },

    // То же в секунду — на это умножается прошедшее время.
    regenRate() {
        return this.regenPerMinute() / 60;
    },
    ...
    fighterHpRaw() {
        ...
        // Регенерация остановлена (идёт бой или забег) — сколько было, столько
        // и есть, сколько бы времени ни прошло.
        if (f.frozen) return f.hp;
        return f.hp + GameTime.secondsSince(f.updated_at) * this.regenRate();
    },

    // Здоровье БЕЗ округления вниз, с потолком. Нужно полосе: при хп в минуту
    // целое число стоит на месте по минуте кряду, и полоса, нарисованная по
    // нему, выглядела бы застывшей. Число под полосой остаётся целым —
    ...
    // «восстанавливается», а не молчать.
