...
// Версия 5: морда переделана по канону РИСОВАННОЙ свиньи — раньше она
// читалась как собачья. Изменены пропорции черепа (широкая челюсть, морда
// почти не выступает вниз), глаза стали заметно мельче и сели ниже, уши
// крупнее. Версия бампнута, чтобы сохранённые персонажи не остались со
// старой (собачьей) посадкой на новой форме головы.
...
function createDefaultEye() {
    return {
        ...
        // Размер здесь — главный рычаг «свинья, а не собака»: крупный глаз с
        // большим белком немедленно уводит морду в собачью/человечью сторону,
        // поэтому scale заметно меньше единицы (было 0.95 — читалось псом).
        offsetX: 23,
        offsetY: -12,
        stretchX: 1,
        stretchY: 0.9,
...
function createDefaultWormModel() {
    return {
        ...
        head: {
            ...
            // Пропорции выставлены по канону РИСОВАННОЙ свиной морды, а не
            // абстрактного «звериного черепа». Что именно отличает свинью от
            // собаки в анфас (и что раньше было сделано наоборот):
            //   • жевательная часть ШИРОКАЯ — челюсть почти не сужается книзу
            //     (у собаки она сходится к узкому носу, отсюда и был «пёс»);
            //   • морда не ВЫСТУПАЕТ вниз длинным бруском, а лишь чуть-чуть
            //     выдаётся — спереди видно в основном сам пятачок и щёки;
            //   • голова шире, чем высокая: самая широкая точка на скулах.
            skull: {
                browWidth: 0.60,   // ширина свода у макушки (лоб между ушами широкий)
                templeWidth: 1.0,  // виски
                cheekWidth: 1.04,  // скулы — самое широкое место, шире высоты
                jawWidth: 0.88,    // челюсть — мясистая, а не сходящаяся клином
                muzzleWidth: 0.66, // ширина морды у пятачка — широкое основание
                chinDrop: 0.98,    // морда почти не выступает вниз
