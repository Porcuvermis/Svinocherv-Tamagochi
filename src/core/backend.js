...
const LocalBackend = {
    ...
    minigameResult(payload) {
        ...
        //
        // alwaysPays в конфиге награды снимает порог у этого исхода. Стоит он
        // сегодня у одного греха — у боя в гневе, и не как поблажка: там
        // тормозом работает здоровье бойца, а два тормоза на одной механике
        // складывались в «дерись сколько хочешь, но платят раз в пять часов»
        // (разбор — ECONOMY.rewards.wrath).
        const pays = reward.alwaysPays ? true : this.sinPays(sin);
        awarded.pays = pays;
        ...
        // Счётчик исходов за сутки: на нём стоит убывающая доходность золота
        // (1–5 побед за сутки дают 100%, 6–15 — половину, дальше — 10%).
        // Двигают его ТОЛЬКО оплаченные заходы: бесплатный повтор при сытом
        // черве ничего не приносит, и было бы странно, если бы он при этом
    ...
    // Ранние ступени сокращают ПОЛИВ (его игрок видит), поздние — ЧАСЫ
    // (их он не ждёт, а уходит) — см. GARDEN.CAN_TIERS.
    canTier() {
        const lvl = this.gardenTools().can;
        return GARDEN.CAN_TIERS[Math.min(lvl, GARDEN.CAN_TIERS.length - 1)];
    },

    // Сколько движений нужно на действие. Число берётся из ступени того
    // инструмента, которым его делают: прокачка сокращает РАБОТУ, а не
    // ожидание, — то же правило, что у лейки (ранние ступени сокращают
    // секунды, которые игрок стоит с инструментом в руках).
    //
    // Возвращается количество ПОЛОВИН цикла: цикл — это «туда и обратно»,
    // а считает экран каждый разворот.
    gardenWorkNeed(action) {
        const cfg = GARDEN.work[action];
        if (!cfg) return 0;
        if (cfg.pulls) return cfg.pulls;
        const lvl = cfg.tool ? this.gardenTools()[cfg.tool] || 0 : 0;
        const cycles = cfg.cycles[Math.min(lvl, cfg.cycles.length - 1)];
        return cycles * 2;
    },

    // Сколько миллисекунд надо держать лейку над грядкой. Спрашивает экран,
    // решает конфиг: длительность действия — такое же число баланса, как
    // цена или скорость роста (инвариант 3).
    gardenPourMs() { return this.canTier().pour; },
    ...
    gardenLeft(bed) {
        ...
        if (bed.stage === 'growing') {
            const hours = this.canTier().hours;
    ...
    gardenAct(i, action, opts) {
        ...
        if (action === 'clear' && bed.stage === 'locked') {
            ...
            bed.stage = 'dug';
            // В земле иногда что-то попадается. Это самое частое действие в
            // саду, и на нём держится единственный ручеёк валюты, не
            // завязанный на чревоугодие: без него игрок, не готовящий еду,
            // остался бы вообще без денег и без магазина (план, раздел 7).
            // Градация редкости — в GARDEN.digFinds.
            res.find = this.gardenDigFind();
            res.ok = true;
    ...
    // останавливается на первой удаче: иначе частая монетка (каждый второй
    // копок) срабатывала бы первой и затеняла собой всё, что за ней, —
    // осколок жетона не выпал бы никогда.
    //
    // Находка ОДНА за копок, даже если повезло дважды: две вещи из одной ямки
    // читаются как сбой, а не как удача.
    gardenDigFind() {
    ...
    // ---------- ГОТОВНОСТЬ АЛЧНОСТИ ДЛЯ КОЛЕСА ГРЕХОВ ----------
    // У шести грехов узел в колесе горит по одному правилу — шкала просела
    // до порога голода. У автомата к этому добавляется ВТОРОЕ условие:
    // монета в кошельке. Пустой кошелёк здесь не «сыграй бесплатно», а
    // «сюда сейчас незачем»: крутка не состоится, шкала не нальётся, и
    // горящий узел обещал бы то, чего не будет.
    //
    // Живёт здесь, а не в колесе: что считать готовностью, знает тот, кто
    // платит (инвариант 2). Колесо только спрашивает.
    greedReady() {
        const cfg = (ECONOMY.minigames && ECONOMY.minigames.greed) || {};
        const max = GameState.maxValue('greed') || 1;
        const value = GameState.sinValue('greed');
        return {
            fill: 100 - (value / max) * 100,
            ready: this.sinPays('greed') && GameState.currency('gold') >= (cfg.spinCost || 0)
        };
    },
    ...
    // ---------- ГОТОВНОСТЬ ГНЕВА ДЛЯ КОЛЕСА ГРЕХОВ ----------
    // У пяти грехов узел в колесе горит по умолчанию — шкала просела до
    // порога голода. У гнева порога нет вовсе (rewards.wrath.duel,
    // alwaysPays): за бой платят всегда, и общее правило показывало бы
    // погасший узел там, где награда на самом деле будет. Колесо, которое
    // врёт в эту сторону, ничем не лучше колеса, обещающего лишнее.
    //
    // Поэтому у гнева своя мера, и это ЗДОРОВЬЕ бойца. Оно и есть тормоз
    // греха, и оно же — то единственное, из-за чего в гнев сейчас может быть
    // рано идти. По лучу течёт заросшее здоровье, узел загорается на полном:
    // бой съедает почти весь запас, и заход побитым — это почти наверняка
    ...
    wrathReady(maxHp) {
        ...
        return { fill: (hp / max) * 100, ready: hp >= max };
    },
    ...
    // где порог и сколько ЧАСОВ до него осталось. Часы считаются из
    // drainHours — той же скорости падения, что и шкала, так что второе
    // место, где эта скорость записана, не появляется.
    //
    // Внутри мини-игры это не показывается и показываться не должно: там
    // правило и так видно — у сытого червя просто нет поцелуйных зон, а
    // цифры «сколько ждать» посреди выхода лишние. Индикация будет ОДНА на
    // все семь грехов и снаружи мини-игр; этот метод — то, из чего она
    // соберётся.
    sinPayInfo(sinKey) {
        const cfg = (ECONOMY.sins && ECONOMY.sins[sinKey]) || {};
