...
(async () => {
    ...
    lines.push('        }).join(\'\');');
    lines.push('        // Каждая заливка ОБВОДИТСЯ СВОИМ ЖЕ цветом. Соседние');
    lines.push('        // треугольники стыкуются ребро в ребро, но сглаживание');
    lines.push('        // svg считает край каждого из них полупрозрачным — и на');
    lines.push('        // стыке остаётся светлая нитка. Из этих ниток и');
    lines.push('        // складывалась «сетка полигонов», которая ползла по');
    lines.push('        // каждой модели. Обводка в единицу перекрывает стык');
    lines.push('        // изнутри и снаружи и не меняет ни формы, ни цвета.');
    lines.push('        return `<g class="bb bb-${name}" stroke-width="1"`');
    lines.push('             + ` stroke-linejoin="round">` + ink');
    lines.push('             + it.d.map(p =>');
    lines.push('                 `<path fill="${p[0]}" stroke="${p[0]}" d="${p[1]}"/>`)');
    lines.push('                 .join(\'\')');
    lines.push('             + en + \'</g>\';');
    lines.push('    },');
    lines.push('');
