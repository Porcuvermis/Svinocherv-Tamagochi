...
const BATH_MODELS = {
    ...
    water(opts) {
        const o = Object.assign({ len: 17, wide: 7.4, deep: 5.2, level: -1.5,
                                  corner: 2.6, seg: 13 }, opts || {});
        const plan = BATH_MODELS.roundedPlan(o.len, o.wide, o.corner, o.seg);
        // Край воды — РОВНО внутренняя стенка на её уровне, посчитанная по
        // профилю. Стояло отдельное число «толщина стенки», и при каждой
        // правке профиля вода то влезала в стенку, то оставляла щель.
        const wall = o.wide / 2 - BATH_MODELS.innerAt(o, o.level);
        // Уровень задан ОТ БОРТА (число отрицательное), а борт стоит на
        // высоте deep над полом — как и сама ванна.
        return BATH_MODELS.flat(
            BATH_MODELS.fan(BATH_MODELS.inset(plan, wall),
                            o.deep + o.level, true));
    },
