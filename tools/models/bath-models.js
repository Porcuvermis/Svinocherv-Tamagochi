...
const BATH_MODELS = {
    ...
    anchorPoints() {
        ...
        return {
            ...
            // Место хвоста задаёт НЕ вкус, а прицел: от кончика до рта
            // должно быть около полутора сотен точек, иначе бросок выходит
            // короче собственной корзины рта и попадает при любом угле
            // (docs/traps.md, п. 32). Рот у повёрнутой влево головы стоит на
            // 389, значит корень хвоста — на 238.
            //
            // Ближе к телу его не подвинуть: проверено, 274 давало 59%
            // попаданий на удержанном прицеле вместо 27%, то есть финал
            // переставал быть игрой. У самой воды туша до хвоста достаёт —
            // брюхо в этом ряду уходит влево до 185.
            tail: { x: -3.0, y: T.deep, z: 0.1 },
