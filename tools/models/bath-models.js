...
const BATH_MODELS = {
    ...
    anchorPoints() {
        ...
        return {
            ...
            // Червь СМОТРИТ ВЛЕВО (opts.flip у рендерера), значит тело уходит
            // вправо и хвост торчит справа от морды. Сторона хвоста — не
            // вкус: он обязан быть с той же стороны, что и остальное тело.
            //
            // Стоит ВПЛОТНУЮ К ТЕЛУ, а не «где-то в стороне от морды»: на полкорпуса
            // врозь хвост и червь читались двумя разными предметами в одном
            // кадре, и глаз не связывал их в одно существо. Теперь основание
            // заходит под силуэт червя — хвост из него и растёт.
            tail: { x: 1.45, y: T.deep, z: 0.1 },
