...
// ---------- СКОЛЬКО ОПЛАЧЕННЫХ ВЫХОДОВ В СУТКИ ----------
// Тормоз против гринда больше не лестница долей, а ПОРОГ ГОЛОДА: за выход
// платят, только пока шкала гордыни опустилась до payAt или ниже
// (ECONOMY.sins.pride). Выход её закрывает целиком, значит следующий
// оплаченный возможен не раньше, чем шкала снова просядет до порога.
//
// Отсюда всё считается одной строкой и без единого коэффициента: сколько
// раз в сутки шкала успевает упасть от максимума до порога — столько выходов
// и оплатят. Всё сверх этого играется бесплатно (и без поцелуйных зон).
const SIN = ECONOMY.sins.pride;
const HOURS_PER_PAID = SIN.drainHours * (1 - SIN.payAt / SIN.max);
const PAID_PER_DAY = 24 / HOURS_PER_PAID;
console.log(`\nпорог голода: платят при шкале ≤ ${SIN.payAt} из ${SIN.max}, ` +
            `падает за ${SIN.drainHours} ч`);
console.log(`значит между оплаченными выходами ${HOURS_PER_PAID.toFixed(1)} ч — ` +
            `${PAID_PER_DAY.toFixed(1)} оплаченных выхода в сутки, сколько бы игрок ни играл`);
...
function daysToBuyAll(perDay, accuracy) {
    ...
    while (bought < steps && day < 3000) {
        ...
        const per = income();
        let dayIncome = 0;
        // Платят ЦЕЛИКОМ, но только за выходы, до которых червь успел
        // проголодаться. Остальные заходы за сутки не приносят ничего — это
        // и есть весь антифарм, и он не отнимает ни одной монеты.
        const paidRuns = Math.min(perDay, PAID_PER_DAY);
        dayIncome += Math.round(per * paidRuns);
        wallet += dayIncome;
...
// Число заходов больше почти ничего не решает — в этом и смысл порога.
// Строки оставлены, чтобы это было ВИДНО: 2 и 40 заходов дают один и тот же
// день, и разойтись они могут только на границе (два захода в сутки — ровно
// столько, сколько успевает проголодаться червь).
for (const perDay of [1, 2, 4, 10, 40]) {
...
console.log('\nпо ступеням (коридор 1.2–1.7 внутри линии; число заходов роли не играет —\nплатят только за те, до которых червь проголодался):');
