...
const TARGET = {
    ...
    // Порог первой покупки — ориентир, а не закон: главное, чтобы первый
    // апгрейд не затягивался. Игрок, чьи усилия уже превратились в ступень,
    // держится за игру; тот, у кого ещё ничего нет, бросает её без сожаления.
    // Поэтому исключения допустимы, но только ОБЪЯВЛЕННЫЕ, поимённо и с
    // причиной — как логотип кухни в инварианте 9.
    //
    // Похоть: низ лестницы хвоста нарочно бедный (0.7 осколка за игру, а на
    // первый жетон нужно три), и каждый пятый заход у среднего игрока
    // попадает на «награда ещё не готова». Выходит четвёртый день — решение
    // автора (docs/plan/21-lust-bath.md, разд. 7а).
    firstBuyDaysBySin: { lust: 4 }
};
...
// ---------- ОПЛАЧЕННЫЕ ЗАХОДЫ ПРИ ТАЙМЕРЕ НАГРАДЫ ----------
// Сколько заходов в сутки оплачено, если игрок заходит `perDay` раз в
// СЛУЧАЙНОЕ время бодрствования (с 8 до 24), а награда открывается через
// `cooldownH` часов после прошлой оплаченной игры. Зерно фиксированное:
// календарь обязан давать одно и то же число при каждом прогоне.
const paidCache = {};
function paidVisits(perDay, cooldownH) {
    const key = perDay + '|' + cooldownH;
    if (paidCache[key] != null) return paidCache[key];
    let seed = 7, last = -1e9, paid = 0;
    const rnd = () => (seed = (seed * 1664525 + 1013904223) >>> 0) / 4294967296;
    const DAYS = 20000;
    for (let d = 0; d < DAYS; d++) {
        const n = Math.floor(perDay) + (rnd() < perDay - Math.floor(perDay) ? 1 : 0);
        const times = Array.from({ length: n }, () => d * 24 + 8 + rnd() * 16).sort((a, b) => a - b);
        for (const t of times) if (t - last >= cooldownH) { paid++; last = t; }
    }
    return (paidCache[key] = paid / DAYS);
}
...
const INCOME = {
    ...
    // ---------- ПОХОТЬ ----------
    // Шкала над головой — жетон из трёх частей разной цены (2, 5, 8
    // попаданий), по осколку за часть. Потолок ЖЁСТКИЙ: один жетон за забег.
    //
    // Осколков за оплаченный забег — по ступени хвоста, таблица
    // tools/sim-lust.js (там средний игрок честно играет финал свайпами).
    // Здесь она не выводится заново: копия модели однажды разошлась бы с
    // оригиналом.
    //
    // Сколько заходов ОПЛАЧЕНО — решает таймер награды (мочалка, 6 → 3 ч), и
    // считается он НЕ как «24 / таймер». Живой игрок заходит когда придётся,
    // а не по расписанию: утром и вечером, иногда дважды за час. Поэтому
    // часть заходов попадает на «награда ещё не готова» — это забег «только
    // помыть», и за него ничего не дают. Равномерные заходы этого не видят
    // вовсе и завышали доход похоти на четверть. Мыло доход не двигает: оно
    // замедляет потребность, а не открывает награду.
    lust(levels, conf) {
        const PER_RUN = [0.67, 0.77, 0.86, 0.92, 1.19, 1.51, 1.81, 2.01, 2.49, 2.89, 2.99];
        const lvl = Math.min(PER_RUN.length - 1, levels.tail || 0);
        const cl = conf.cloth, cl_lvl = Math.min(levels.cloth || 0, cl.levels.length);
        const cooldown = cl_lvl ? cl.levels[cl_lvl - 1].bonus.cooldown : cl.base.cooldown;
        return paidVisits(visits, cooldown) * PER_RUN[lvl] / ECONOMY.exchange.lust_shard.per;
    },
...
target.runs.forEach(r => {
    const firstMax = (TARGET.firstBuyDaysBySin || {})[r.sin] || TARGET.firstBuyDays;
    check(r.firstBuy <= firstMax,
          `${r.sin}: первая покупка на ${r.firstBuy}-й день (порог ${firstMax})`);
