...
const INCOME = {
    ...
    //
    // Доход двигает ОДНА лестница — хвост. Мыло и мочалка доход не двигают:
    // их вторая половина (реже заходить, раньше награда) ещё не сделана, и
    // когда будет — изменит здесь число оплаченных заходов, а не осколки.
    //
    // Осколков за забег по ступеням хвоста — таблица tools/sim-lust.js (там
    // средний игрок честно играет финал свайпами). Здесь она не выводится
    // заново: копия модели однажды разошлась бы с оригиналом.
    lust(levels, conf) {
        const PER_RUN = [0.70, 0.78, 0.85, 0.91, 1.17, 1.45, 1.78, 1.99, 2.41, 2.86, 2.99];
        ...
        return Math.min(visits, runsCap) * PER_RUN[lvl] / ECONOMY.exchange.lust_shard.per;
    },
