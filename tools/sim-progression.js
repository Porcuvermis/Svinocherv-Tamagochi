...
const INCOME = {
    ...
    // ---------- ПОХОТЬ ----------
    // Осколок падает за КАЖДУЮ заполненную секцию финальной шкалы, секций
    // три, три осколка — жетон. То есть потолок ЖЁСТКИЙ: ровно один жетон за
    // забег, сколько ни целься.
    //
    // Доход двигает ОДНА ветка из четырёх — прицел. Мыло, мочалка и масло
    // укорачивают дорогу к финалу и на осколки не влияют вовсе: мытьё не
    // источник дохода, и это граница замысла, а не недоделка.
    //
    // Числа взяты у tools/sim-lust.js (1.09 осколка за забег на старте,
    // 2.97 на девятой ступени) и здесь не выводятся заново: там капля честно
    // интегрируется шагом в шестидесятую секунды, и повторять эту модель
    // значило бы завести ей вторую копию.
    lust(levels, conf) {
        const aimMax = conf.aim.levels.length;
        const share = aimMax ? Math.min(1, (levels.aim || 0) / aimMax) : 0;
        const shards = 1.09 + (2.97 - 1.09) * share;
        const runsCap = 24 / (ECONOMY.sins.lust.drainHours *
                              (1 - ECONOMY.sins.lust.payAt / ECONOMY.sins.lust.max));
        return Math.min(visits, runsCap) * shards / ECONOMY.exchange.lust_shard.per;
    },
