...
// Верхняя ступень ветки — для сверки с таблицей выше: по ней видно, до какой
// скорости вообще можно докачаться. На сколько дней растянута вся лестница и
// не схлопывается ли бой к её концу, считает tools/sim-wrath-year.js.
const top = W.upgrades.regen.levels[W.upgrades.regen.levels.length - 1];
const topHp = W.upgrades.hp.levels[W.upgrades.hp.levels.length - 1];
console.log(`\nпотолок веток: регенерация ${W.upgrades.regen.levels.length} ступеней `
  + `до ${(W.regenPerMinute + top.bonus).toFixed(1)} хп/мин, `
  + `здоровье до ${W.baseHp + topHp.bonus} хп без снаряжения.`);
